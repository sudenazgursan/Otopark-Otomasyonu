...
namespace Sudenaz_Gürsan_22MY03039
{
    internal class Otopark
    {
        ...
        {
            bool kontrolplaka = false;
            foreach (var arac in otopark)
            {
                if (arac.Plaka== plaka)
                {
                    // Plaka kontrolünü true yapar ve döngüden çıkar.   
                    kontrolplaka = true;
                    break;       
                }
            }

            if (!kontrolplaka && otopark.Count() < kapasite) // Plaka kontrolü false ise ve otopark kapasitesi dolmadıysa devam eder.
            {
